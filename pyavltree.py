...
class AVLTree():
    ...
    def min_node(self):
        return self.find_smallest(self.rootNode)
    ...
    #find closest two elment -- the closest bigger one and the closest smaller one (bigger_node, smaller_node),
    # if find equal elment, then return (equal_node, None))
    def find_closest_nodes(self, key):
        bigger_or_equal_node = None
        smaller_node = None
        return self.find_closest_node_in_subtree(self.rootNode, key)

    def find_closest_node_in_subtree(self, node, key):
        bigger_or_equal_node = None
        smaller_node = None
        return self.find_closest_node_in_subtree_internal(node, key, bigger_or_equal_node, smaller_node)

    def find_closest_node_in_subtree_internal(self, node, key, bigger_or_equal_node, smaller_node):
        if node is None:
            return (bigger_or_equal_node, smaller_node)
        if node.key == key:
            bigger_or_equal_node = node
            smaller_node = None
            return (bigger_or_equal_node, smaller_node)
        elif node.key < key:
            # the biggest smaller node is the cloest smaller node
            if smaller_node is None or node.key > smaller_node.key:
                smaller_node = node
            return self.find_closest_node_in_subtree_internal(node.rightChild, key, bigger_or_equal_node, smaller_node)
        else:
            # the smallest bigger node is the cloest bigger node
            if bigger_or_equal_node is None or node.key < bigger_or_equal_node.key:
                bigger_or_equal_node = node
            return self.find_closest_node_in_subtree_internal(node.leftChild, key, bigger_or_equal_node, smaller_node)
